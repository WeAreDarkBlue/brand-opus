...
export default class FluidSimulation {
  ...
  density: { read: WebGLRenderTarget; write: WebGLRenderTarget }
  velocity: { read: WebGLRenderTarget; write: WebGLRenderTarget }
  pressure: { read: WebGLRenderTarget; write: WebGLRenderTarget }
  ...
  constructor({ renderer, size = 128 }) {
    if (!renderer) {
      console.error('FluidSimulation: renderer is required')
      return
    }
