...
console.log('head',data)
	...
	return (
		<Sheet open={open} onOpenChange={setOpen}>
			...
			>
				<Container noPadding>
					<div className={cn("flex items-center justify-between lg:h-24 px-[15px] lg:px-15 py-4 lg:py-15")}>
						<SanityLink
							link="/"
							className="block w-[84px] md:w-[120px] mix-blend-difference"
						>
				...
				<SheetContent side={"left"} className="!bg-[#ff530f]/[0.9] w-full h-screen flex flex-col justify-between px-4 py-4">
					<SheetTitle className="hidden">Sheet</SheetTitle>
						...
						<ul className="list-style-none" ref={navEl}>
					...
					<div>
					<ul className="list-style-none flex flex-row gap-3">
						{lowerLinks.map((link) => (
							<li key={link._key}>
								<SanityLink
									link={link.slug}
			...
			{/* <button onClick={toggleSheet} className="absolute right-2 top-2 z-[100] text-white">Toggle Sheet</button> */}
